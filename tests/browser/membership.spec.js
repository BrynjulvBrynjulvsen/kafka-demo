...
test('shared membership widget selects groups and shows transitions, assignments and stale state', async ({ page }) => {
  ...
  expect(commands.pop()).toEqual({action:'delay',group:'demo-b',delayMs:200});
  await widget.getByRole('button',{name:'+ Add member'}).click();
  expect(commands).toEqual([{action:'start',group:'demo-b',policy:'earliest'}]);
  ...
  expect(commands[1]).toEqual({action:'stop',group:'demo-b'});
